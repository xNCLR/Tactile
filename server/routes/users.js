...
router.put('/profile', authenticate, validate(updateProfileSchema), async (req, res) => {
  try {
    ...
    if (latitude && longitude) {
      runSql(db, `UPDATE users SET name = COALESCE(?, name), phone = COALESCE(?, phone), postcode = COALESCE(?, postcode), latitude = ?, longitude = ?, updated_at = datetime('now') WHERE id = ?`,
        [name || null, phone || null, postcode || null, latitude, longitude, req.user.id]);
    } else {
